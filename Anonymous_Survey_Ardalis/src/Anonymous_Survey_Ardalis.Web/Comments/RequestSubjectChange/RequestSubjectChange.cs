...
public class RequestSubjectChange : Endpoint<RequestSubjectChangeRequest, RequestSubjectChangeResponse>
{
    ...
    public override void Configure()
    {
        ...
        Summary(s =>
        {
            ...
            s.Description = "Subject admins can request to change a comment's subject if it was submitted under the wrong subject";
        });
    }
    ...
    public override async Task HandleAsync(RequestSubjectChangeRequest request, CancellationToken cancellationToken)
    {
        try
        {
            var command = new RequestSubjectChangeCommand
            {
                ...
                SuggestedSubjectId = request.SuggestedSubjectId,
            ...
            if (result.IsSuccess)
            {
                ...
                {
                    Success = true,
                    Message = "Subject change request sent successfully"
                };
            }
            else
            {
                ...
                {
                    Success = false,
                    Message = result.Errors.FirstOrDefault() ?? "Failed to send subject change request"
                };
