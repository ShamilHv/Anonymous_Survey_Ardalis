...
[Authorize]
public class
  ReportInappropriateComment : Endpoint<ReportInappropriateCommentRequest, ReportInappropriateCommentResponse>
{
  ...
  public override async Task HandleAsync(ReportInappropriateCommentRequest request, CancellationToken cancellationToken)
  {
    try
    {
      var command = new ReportInappropriateCommentCommand { CommentId = request.CommentId, Message = request.Message };
      ...
      if (result.IsSuccess)
      {
        ...
        {
          Success = true, Message = "Inappropriate comment report sent successfully"
        };
      }
      else
      {
        Response = new ReportInappropriateCommentResponse
        {
          Success = false, Message = result.Errors.FirstOrDefault() ?? "Failed to report inappropriate comment"
        };
    ...
    {
      Response = new ReportInappropriateCommentResponse { Success = false, Message = ex.Message };
