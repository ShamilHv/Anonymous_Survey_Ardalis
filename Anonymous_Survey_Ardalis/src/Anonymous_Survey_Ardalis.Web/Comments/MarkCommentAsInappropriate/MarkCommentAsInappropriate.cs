...
[Authorize]
public class MarkCommentAsInappropriate : Endpoint<MarkCommentAsInappropriateRequest, MarkCommentAsInappropriateResponse>
{
    ...
    public override async Task HandleAsync(MarkCommentAsInappropriateRequest request, CancellationToken cancellationToken)
    {
        try
        {
            var command = new MarkCommentAsInappropriateCommand
            {
            ...
            if (result.IsSuccess)
            {
                ...
                {
                    Success = true,
                    Message = "Comment marked as inappropriate successfully"
                };
            }
            else
            {
                ...
                {
                    Success = false,
                    Message = result.Errors.FirstOrDefault() ?? "Failed to mark comment as inappropriate"
                };
