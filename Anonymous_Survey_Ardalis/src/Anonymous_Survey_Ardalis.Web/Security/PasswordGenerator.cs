...
public static class PasswordGenerator
{
  ...
  public static string GenerateSecurePassword(int length = 12, bool includeSpecial = true)
  {
    ...
    var characterPool = LowerCase + UpperCase + Numbers;
    if (includeSpecial)
    {
    ...
    for (var i = includeSpecial ? 4 : 3; i < length; i++)
    {
      var index = randomBytes[i] % characterPool.Length;
  ...
  private static string ShuffleString(string input, byte[] randomSource)
  {
    ...
    {
      var swapIndex = randomSource[i] % (i + 1);
      if (swapIndex != i)
      {
